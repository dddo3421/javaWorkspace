...
public class ClassCoffeeManager {
	...
	public static void main(String[] args) {
		...
		Coffee[] coffees = { new Coffee("americano", 1000, 500, 10, 3, 0), 
			...
			try {
				...
				if (menu == 1) {
					...
					System.out.println("\n 제품명 : " + coffees[drinkNo].getName() + "\n 판매가 : "
							+ coffees[drinkNo].getPrice() + "\n 판매수량 : " + orderCnt + "\n 결재금액 : "
							+ orderCnt * coffees[drinkNo].getPrice() + "\n 남은 재고 : " + coffees[drinkNo].getStock());

				} else {
					System.out.println("\n==== info ====\n");

					for (int i = 0; i < coffees.length; i++) {
						System.out.printf("%-10s 재고(%3d) 판매량(%3d) \n", coffees[i].getName(), coffees[i].getStock(),
								coffees[i].getSalesAmount());
					}

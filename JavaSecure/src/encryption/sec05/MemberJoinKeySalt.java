...
import java.security.SecureRandom; //암호화 시 사용권장되는 메서드
...
public class MemberJoinKeySalt {
	//salt크기 설정
	private static final int SALT_SIZE = 16; //salt 난수 16개
	...
	public static String Byte_to_String(byte[] tmp) {
		...
		for (byte b : tmp) {
			sb.append(String.format("%02x", b)); // %02x → 바이트를 두 자리의 16진수(0패딩 포함)로 변환
		}
		return sb.toString();
	}
	...
	public static String Hashing(byte[] password, String Salt) throws Exception {
		...
		for (int i = 0; i < 10000; i++) {
			// password 바이트 → 16진수 문자열로 변환 + salt 붙이기
			String temp = Byte_to_String(password) + Salt; // password 바이트 → 16진수 문자열로 변환 + salt 붙이기
			md.update(temp.getBytes()); // 결합된 문자열을 다시 SHA-256 입력값으로 사용
			password = md.digest(); // 새로운 digest 값으로 password 갱신
		}
		return Byte_to_String(password); // 최종 digest를 16진수 문자열로 변환 후 반환
	}
